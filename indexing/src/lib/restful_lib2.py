...
class Connection:
    def __init__(self, base_url, username=None, password=None):
        ...
        self.h = httplib2.Http(config['tinyurl_cache'])
    ...
    def request(self, resource, method = "get", args = None, body = None, filename=None, headers={}):
        ...
        full_request_path = u"%s://%s%s" % (self.scheme, self.host, u'/'.join(request_path))
        resp, content = self.h.request( full_request_path, method.upper(), body=body, headers=headers )
        return {u'headers':resp, u'body':content.decode('UTF-8'), u'request':full_request_path}
